"""
Merge Meshy per-animation GLBs for The Ripped Bride biped into one ghost.glb with all clips.

Filenames:
  Meshy_AI_The_Ripped_Bride_biped_Animation_<ClipName>_withSkin.glb
→ action / glTF clip name <ClipName>.

Uses **Walking** as bind-pose base (calm idle); bride pack has no Idle_* clip.

  blender --background --python scripts/blender_merge_ghost_animation_glbs.py -- \\
    "/path/to/Meshy_AI_The_Ripped_Bride_biped" \\
    --output "/path/to/ghost-merged-temp.glb"

Then retarget node indices:
  node scripts/retarget-glb-animation-nodes.mjs ghost-merged-temp.glb public/models/monsters/ghost.glb --ref Walking
"""
...
def list_input_glbs(directory: Path) -> list[Path]:
    paths = sorted(directory.glob("Meshy_AI_The_Ripped_Bride_biped_Animation_*_withSkin.glb"))
    if not paths:
...
def main() -> None:
    ...
    if not in_dirs or out_glb is None:
        print(
            "Usage: blender --background --python scripts/blender_merge_ghost_animation_glbs.py -- "
    ...
    idle = next((p for p in paths if "Walking" in p.name), paths[0])
    ...
    print(
        ...
        f'"{out_glb}" public/models/monsters/ghost.glb --ref Walking'
    )
