...
const STATE_NOTES: Record<Monster3DSpriteState, string> = {
  ...
  hurt: "Monster took a hit (player won the roll). Dracula 3D: reaction clip scales with HP left / max (see table).",
    ...
    >
      <main style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        <p style={{ marginBottom: 24, color: "#a89cb0", fontSize: "0.95rem" }}>
          ...
          <code style={{ color: "#c4b8d4" }}>NEXT_PUBLIC_MONSTER_3D=1</code>, combat shows glTF. <strong>Dracula</strong> uses
          one merged <code style={{ color: "#c4b8d4" }}>dracula.glb</code>: the model stays loaded; portrait state only
          changes which clip the mixer plays (cross-fades in <code style={{ color: "#c4b8d4" }}>MonsterModel3D</code>). Clip
          try-order matches <code style={{ color: "#c4b8d4" }}>lib/monsterModels3d.ts</code>. Framing: combat uses{" "}
          <code style={{ color: "#c4b8d4" }}>referenceViewerStyle</code>; this preview uses the tight-framing checkbox.
        </p>
            ...
            <thead>
              <tr style={{ background: "rgba(255,152,103,0.12)", textAlign: "left" }}>
                ...
                <th style={{ padding: "10px 12px", borderBottom: "1px solid rgba(255,255,255,0.08)", minWidth: 220 }}>
                  Dracula <code style={{ color: "#c4b8d4" }}>(V)</code> — <code>dracula.glb</code> + clip tries
                </th>
                <th style={{ padding: "10px 12px", borderBottom: "1px solid rgba(255,255,255,0.08)", minWidth: 200 }}>
